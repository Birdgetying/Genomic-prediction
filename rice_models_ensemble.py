"""
Rice Genomic Prediction — FGN + MICNN Ensemble
=====================================================
"局部-全局" 正交集成体系
...
对照: 保留 FGN/MICNN 原版作为基线
"""

import json, time, pickle, shutil, random
import numpy as np
...
def main():
    ...
    for t_idx, trait in enumerate(traits_run):
        ...
        for fi, (tr, te) in enumerate(kf.split(X_all)):
            ...
            Xtr_s = sc_gwas.fit_transform(Xtr_gwas).astype(np.float32)
            Xte_s = sc_gwas.transform(Xte_gwas).astype(np.float32)
            ...
            if MARKER_SELECTOR == 'haplotype':
                ...
                gidx_dl = hybrid_select(Xtr_raw, ytr, n_snps,
                                        gwas_frac=HAPLO_GWAS_FRAC)
            else:
                gidx_dl = gidx_gwas  # 默认使用 GWAS
            ...
            # ── ResFGN: Ridge + FusionNet residual learning (DL markers) ──
            t0 = time.time()
        ...
        # ── DL 模型: 可配置标记筛选 ──
        if MARKER_SELECTOR == 'haplotype':
            gidx_dl = haplotype_select(X_all, y, n_snps)
        elif MARKER_SELECTOR == 'hybrid':
            gidx_dl = hybrid_select(X_all, y, n_snps,
                                    gwas_frac=HAPLO_GWAS_FRAC)
        else:
            gidx_dl = gidx_gwas
        X_dl = X_all[:, gidx_dl]
        ...
        # Traditional models — fit on GWAS markers
